...
import { ArrowRight, Check, CircleAlert, Clock3, Cookie, FileText, Globe2, LockKeyhole, Radar, ShieldCheck, ShieldAlert, Server, Settings2, X, Zap } from "lucide-react";
...
type Result = {
  ...
  tls:{https:boolean;hsts:boolean;certificate:string};
  dns:{A:string[];AAAA:string[];MX:string[];NS:string[];TXT:string[];CAA:string[]};
...
const steps = [
  ...
  ["cors","CORS analysis"],["files","robots.txt / security.txt"],["technology","Technology detection"],["tls","HTTPS / TLS posture"],
] as const;
...
export default function Home() {
  ...
  async function scan() {
    ...
    try {
      if (!API) throw new Error("NEXT_PUBLIC_API_URL is not configured. Set it to your deployed WebSentry Worker URL.");
      const response = await fetch(`${API.replace(/\/$/,"")}/api/scan`, { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({url}) });
      if (!response.ok) {
  ...
  function reset() { setResult(null); setError(""); setProgress({}); setUrl(""); window.scrollTo({top:0,behavior:"smooth"}); }
  ...
  return <main className="shell">
    ...
    <section className="hero" id="scanner">
      <div>
        ...
        <p>WebSentry performs a real, ephemeral security analysis of a public website. Inspect headers, HTTPS posture, cookies, CORS, DNS, redirects, common files and detected technologies — without creating an account.</p>
        <div className="scanbar">
          <Globe2 size={20} style={{margin:"15px 0 0 13px",color:"#6d7b8f"}}/>
        ...
        {error && <div className="errorbox"><strong>Scan error:</strong> {error}</div>}
      </div>
    ...
    {(running || result) && <section className="section" id="report">
      {running && <div className="panel"><div className="scanhead"><div><h2>Scanning {url}</h2><div style={{color:"#718097",fontSize:12,marginTop:4}}>Results are generated in real time and are not persisted by the application.</div></div><span className="statuspill">Live scan</span></div><div className="progressbody"><div className="progressbar"><div style={{width:`${Math.max(4,percent)}%`}}/></div><div className="progressmeta"><span>{completed} of {steps.length} checks complete</span><span>{percent}%</span></div><div className="steps">{steps.map(([key,name]) => <div className={`step ${progress[key]==="complete"?"done":progress[key]==="running"?"running":""}`} key={key}><div className="stepicon">{progress[key]==="complete"?<Check size={15}/>:progress[key]==="running"?<Radar size={15}/>:<span>•</span>}</div>{name}</div>)}</div></div></div>}
      {result && <Report result={result} onNew={reset}/>} 
    </section>}
    ...
    {!running && !result && <section className="section" id="features"><div className="panel" style={{padding:30}}><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>{[
      [ShieldCheck,"Security headers","CSP, HSTS, X-Frame-Options, Referrer-Policy and more."],[Globe2,"Network posture","DNS records, redirects, HTTP status and response metadata."],[Cookie,"Cookie security","Inspect Secure, HttpOnly and SameSite attributes."],[Settings2,"Web technologies","Detect common frameworks, servers and client tooling."],
    ].map(([Icon,title,text]) => <div className="card" key={String(title)}><Icon size={22} color="#1769ff"/><div className="cardtitle" style={{marginTop:12}}>{String(title)}</div><div style={{color:"#65738a",fontSize:13,lineHeight:1.6}}>{String(text)}</div></div>)}</div></div></section>}

    <section className="section" id="how"><div className="panel" style={{padding:30}}><div className="eyebrow">How it works</div><h2 style={{fontSize:32,margin:"0 0 10px",letterSpacing:"-.04em"}}>A real scan, returned directly to your browser.</h2><p style={{color:"#65738a",maxWidth:760,lineHeight:1.7,marginTop:0}}>The browser sends the target to a Cloudflare Worker. The Worker validates the URL, blocks private/reserved targets, resolves DNS, performs bounded public HTTP requests and streams each completed check back to the browser. There is no application database and no scan-history feature.</p></div></section>
    <section className="section" id="privacy"><div className="panel" style={{padding:30}}><div className="eyebrow">Privacy model</div><h2 style={{fontSize:32,margin:"0 0 10px",letterSpacing:"-.04em"}}>No login. No database. No stored reports.</h2><p style={{color:"#65738a",lineHeight:1.7,maxWidth:850}}>WebSentry does not intentionally persist scan results or target history. Operational infrastructure such as hosting and network providers can still have their own logs, so this site does not claim that network traffic is invisible everywhere.</p></div></section>
    <footer style={{padding:"30px 5vw 55px",textAlign:"center",color:"#8290a4",fontSize:12}}>WebSentry · Use only on websites you own or are authorized to assess.</footer>
  </main>;
}

function Report({ result, onNew }: { result: Result; onNew: () => void }) {
    ...
    <div className="panel">
      <div className="results">
        <aside className="sidebar">
          {[
            [ShieldCheck, "Overview"], [ShieldAlert, "Security headers"], [LockKeyhole, "HTTPS / TLS"],
            [Globe2, "DNS records"], [Cookie, "Cookies"], [Radar, "CORS"], [ArrowRight, "Redirects"],
            [FileText, "robots.txt"], [FileText, "security.txt"], [Server, "Technologies"], [CircleAlert, "All findings"],
          ].map(([Icon, label], i) => {
            const I = Icon as typeof ShieldCheck;
            return <div className={`sideitem ${i === 0 ? "active" : ""}`} key={String(label)}><I size={15} />{String(label)}</div>;
          })}
        </aside>
        ...
        <div className="report">
          <div className="reporttop">
            ...
            <div className="actions"><button className="secondary" onClick={onNew}>New Scan</button></div>
          </div>
          ...
          <div className="cards" style={{ marginTop: 14 }}>
            <div className="card">
              <div className="cardtitle">Security Headers</div>
            ...
            <div className="card">
              ...
              {Object.entries(result.dns).map(([key, values]) => <div className="metric" key={key}><span>{key}</span><strong>{values.length}</strong></div>)}
            </div>

            <div className="card">
              <div className="cardtitle">Site Details</div>
              <div className="metric"><span>Technologies</span><strong>{result.technologies.length}</strong></div>
              <div className="metric"><span>Cookies</span><strong>{result.cookies.length}</strong></div>
              <div className="metric"><span>robots.txt</span><strong>{result.files.robots.found ? "Found" : "Not found"}</strong></div>
              <div className="metric"><span>security.txt</span><strong>{result.files.securityTxt.found ? "Found" : "Not found"}</strong></div>
            </div>
          </div>

          <div className="findings">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div className="cardtitle" style={{ margin: 0 }}>Findings</div>
              <select value={filter} onChange={(e) => setFilter(e.target.value as Severity | "all")} style={{ border: "1px solid #e4eaf2", borderRadius: 8, padding: "7px 10px", color: "#536279", background: "white" }}>
